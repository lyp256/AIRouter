...
- **后端 (Go)**:
  ...
  - `internal/service/`: 核心逻辑 (upstream_selector, retry)
...
- **前端 (Vue 3 + TS + Tailwind)**:
  ...
  - `web/src/utils/format.ts`: credits 单位格式化与转换逻辑

## 数据模型精要
- **Model (对外模型)**: `name` 唯一，不绑定协议类型。
- **Upstream (上游模型)**: 负载均衡基本单位。关联 `ModelID`, `ProviderID`, `ProviderKeyID`，协议类型来自关联的 Provider。
- **credits 单位**: 
  - **后端存储**: `int64` nano credits/K tokens (每千 token 价格)
  - **前端显示**: credits/M tokens (每百万 token 价格)
  - **换算**: 存储值 (nano credits/K) * 10^6 = 显示值 (credits/M)；1 credit = 10^9 nano credits。
